...
test('debtClose', () => {
  ...
  expect(textareaOutput).toHaveTextContent(expected);
  expect(textareaOutput.textContent).toMatch('db_admin');
});

test('driver', () => {
  input = ['ОАО ПАТП-4 (Новокузнецк),водитель,Ферг Нерман Сбербанкович,,123,1\nВан-Тин-Фан Александр Александрович 123456'];
  ...
  const expected = `${csvHeader} ${'ОАО ПАТП-4 (Новокузнецк),водитель,Ферг,Нерман,Сбербанкович,,123,1'} ${
...
test('grz', () => {
  ...
  const expected = `${'Нет русских букв AY23442 AP92342 AX51942E'}`;
  expect(textareaOutput).toHaveTextContent(expected);
});

test('term', () => {
  input = ['22223333,123,1234,1,3,,true,321,111111111111,2222,22223333,3333,4111,643,J1,Q,1,TKP000000001\n12345678'];
  ...
  const expected = `${csvHeader} ${'22223333,123,1234,1,3,,true,321,111111111111,2222,22223333,3333,4111,643,J1,Q,1,TKP000000001'} ${
